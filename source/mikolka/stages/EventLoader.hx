...
class EventLoader extends BaseStage {
	...
	public static function implement(funk:FunkinLua) {
		...
		Lua_helper.add_callback(lua, "changeTransStickers", function(stickerSet:String = null, stickerPack:String = null) {
			if (stickerSet != null && stickerSet != "")
				StickerSubState.STICKER_SET = stickerSet;
			if (stickerPack != null && stickerPack != "")
	...
	public static function addstage(name:String) {
		...
		if (VsliceOptions.LEGACY_BAR)
			new LegacyScoreBars();
		new VSliceEvents();
		if (name == "tank" || name == "tankmanBattlefieldErect")
		...
		currentStage = switch (name) {
			...
			case 'hotelCastle': new HotelCastleStage();
			default: null;
		};
